...
function InventoryListItem(props) {
  ...
  const [itemInCart, setItemInCart] = useState(ShoppingCart.isItemInCart(props.id));
  ...
  if (itemInCart) {
    cartButton = <button className="btn_secondary btn_inventory" onClick={() => removeFromCart(id)}>REMOVE</button>;
  } else {
    cartButton = <button className="btn_primary btn_inventory" onClick={() => addToCart(id)}>ADD TO CART</button>;
  }
